...
def build_link_relevance_prompt(candidates: list[dict], objective: str) -> str:
    """Judge in-page links by NAME (anchor text + URL) for crawl-worthiness.

    Each candidate is ``{"url": ..., "name": <anchor text>}``. The model decides, from the
    name and URL alone, which links plausibly lead to content matching the objective —
    spending tokens to gain accuracy over a blind regex allow-list. It must echo only URLs
    present in CANDIDATES.
    """

    return dedent(
        f"""
        You are the link-relevance judge for a legal and regulatory crawler.

        Decide which of the candidate links are worth following, judging ONLY by each
        link's visible name (anchor text) and its URL. Keep links whose name or URL slug
        plausibly leads to content matching the CRAWL_OBJECTIVE — laws, regulations,
        guidance, decisions, publications, and document indexes. Drop navigation chrome,
        account, login, search, contact, social, language, share, print, and unrelated
        links. When a name is ambiguous, use the URL slug as the tiebreaker.

        Return JSON only. Put in selected_urls ONLY exact URLs that appear in CANDIDATES.

        CRAWL_OBJECTIVE: {objective}
        Schema: {_schema_block(LINK_RELEVANCE_SCHEMA)}
        CANDIDATES:
